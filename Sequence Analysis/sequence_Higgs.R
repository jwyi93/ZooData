...
setwd("~/Desktop/Sequence/Higgs Sequence/Data")
Annotation_History <- read.csv("MixHiggsAnnotations.csv")
...
Annotation_History_100Sample_Cast_Plot[is.na(Annotation_History_100Sample_Cast_Plot)] <- "nr"
...
anno.seq <- seqdef(Annotation_History_100Sample_Cast_Plot, 1:99, alphabet = annon.alphabet, states = annon.scodes, 
...
setwd("/Users/coreyjackson/Desktop/Sequence/Higgs Sequence/Figures/")
pdf("higgs_sample_sequences1.pdf", height=5, width=10) # Need to change plot name
seqiplot(anno.seq, withlegend = FALSE, border = NA)
dev.off()

pdf("higgs_sample_sequences2.pdf", height=5, width=10) # Need to change plot name
seqIplot(anno.seq, sortv = "from.start", withlegend = FALSE)
dev.off()

anno.om <- seqdist(anno.seq, method = "OM", indel = 1, sm = "TRATE",with.missing=TRUE)
...
# Graph for clusters
pdf("higgs_sample_sequences3.pdf", height=5, width=10) # Need to change plot name
...
annon.labels <- c("Anonymous User","Registered User","Session End")
annon.scodes <- c("AU","RU","END")
anno.seq.full <- seqdef(Annotation_History_FULL, 2:1313, alphabet = annon.alphabet, states = annon.scodes, 
    labels = annon.labels, xtstep = 3)

#Graphs
pdf("higgs_all_sequences1.pdf", height=5, width=10) # Need to change plot name
seqiplot(anno.seq.full, withlegend = FALSE, border = NA)
dev.off()

pdf("higgs_all_sequences2.pdf", height=5, width=10) # Need to change plot name
...
# Graph for clusters
pdf("higgs_all_sequences3.pdf", height=5, width=10) # Need to change plot name
...
setwd("/Users/coreyjackson/Desktop/Sequence/Higgs Sequence/Data")
write.csv(seq.full,"Higgs_RawSequences.csv")
